...
```
```{python}
# separating the arrays into x and y
first_set = apple_cm_arr[:, :, 0]  # Shape (5724, 39)
second_set = apple_cm_arr[:, :, 1]  # Shape (5724, 39)

# combining them back into one longer array
combined = np.concatenate((first_set, second_set), axis=1)  # Shape (5724, 78)
...
landmark.to_csv("20240912landmark_points.csv")
```
...
mdata.to_csv("20240912apple_metadata.csv", index = False)
```
...
apple_traits <- read.csv("20240912apple_metadata.csv")
...
write.csv(apple_traits, file = "20240912apple_metadata_avg.csv")
```
...
apple_traits <- read.csv("20240912apple_metadata_avg.csv") %>% 
  select(2:16)
...
apple_data <- read.csv("20240912apple_metadata_avg.csv") %>% 
  select(2:16)
...
write.csv(all_data, file = "20240912apple_metadata.csv")
```
...
new_mdata = pd.read_csv("20240912apple_metadata.csv")
...
new_mdata.to_csv("20240912apple_metadata.csv", index = False)
```
```{r}
setwd("C:/Users/User/Desktop/COOP02/code/apple_shape")
apple_traits <- read.csv("20240912apple_metadata.csv")
...
predicted <- read.delim("prediction_values.txt", sep = "")
apple_data <- read.csv("20240912apple_metadata.csv") %>% 
  select(2:16, 22:28)
...
ggsave("0912_pairs_plot.png", plot = last_plot(), units = "in", width = 17, height = 15)
write.csv(all_data, file = "20240912apple_metadata.csv")
```
...
avgdata <- read.csv("20240912apple_metadata.csv")
...
write.csv(combined, file = "20240912apple_metadata.csv", row.names = FALSE)
```
...
averaged = pd.read_csv("20240912apple_metadata.csv")
...
averaged.to_csv("20240912apple_metadata.csv", index = False)
```
...
setwd("C:/Users/User/Desktop/COOP02/code/apple_shape")
data <- read.csv("20240912landmark_points.csv")
...
lda_apple <- lda(apple_id~x1+y1+x2+y2+x3+y3+x7+y7+x8+y8+x9+y9+x10+y10+x11+y11+x12+y12+x13+y13+x14+y14+x15+y15+x16+y16+x17+y17+x18+y18+x19+y19, data=data, CV=TRUE)

#Create a dataframe with actual and predicted species

actual_spe <- as.data.frame(top_data$apple_id)
...
#m_spe_predict$actual_spe <- factor(m_spe_predict$actual_spe, levels=c())

#m_spe_predict$predicted_spe <- factor(m_spe_predict$predicted_spe, levels=c())
...
#+ geom_vline(xintercept=c(2.5,9.5,19.5,23.5,27.5,33.5)) + geom_hline(yintercept=c(2.5,9.5,19.5,23.5,27.5,33.5)) + coord_fixed()
