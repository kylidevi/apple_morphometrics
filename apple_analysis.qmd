...
apple_data <- read.csv("apple_metadata_avg_0903.csv") %>% 
  select(2:16)
...
write.csv(all_data, file = "apple_metadata_0905.csv")
```
...
new_mdata = pd.read_csv("apple_metadata_0905.csv")
...
new_mdata.to_csv("apple_metadata_0905.csv", index = False)
```
```{r}
setwd("C:/Users/User/Desktop/COOP02/code/apple_shape")
apple_traits <- read.csv("apple_metadata_0905.csv")
apple_traits <- apple_traits %>% 
  separate(file, into = c("BW", "nursery_id", "ending"), sep = "_", remove = FALSE) %>% 
  select(2, 4, 6:30)
...
write.csv(apple_traits, file = "apple_metadata_avg_0905.csv")
```
...
apple_traits <- read.csv("apple_metadata_avg_0905.csv") %>% 
  select(2:28)
...
predicted <- read.delim("prediction_values.txt", sep = "")
apple_data <- read.csv("apple_metadata_0905.csv") %>% 
  select(2:16, 22:28)
...
ggsave("0905_pairs_plot.png", plot = last_plot(), units = "in", width = 17, height = 15)
write.csv(all_data, file = "apple_metadata_0905.csv")
```
...
avgdata <- read.csv("apple_metadata_0905.csv")
dataset <- read_excel("Dataset_S1.xlsx", sheet = "phenotype_data")

combined <- avgdata %>% left_join(dataset, by = 'apple_id')

write.csv(combined, file = "apple_metadata_0905.csv", row.names = FALSE)
```
...
averaged = pd.read_csv("apple_metadata_0905.csv")
...
plt.gca().get_legend().remove()
plt.colorbar(cmap="viridis", label="Aspect Ratio")
xlab = "PC1, " + str(round(pca.explained_variance_ratio_[0]*100,1)) + "%"
ylab = "PC2, " + str(round(pca.explained_variance_ratio_[1]*100,1)) + "%"
plt.xlabel(xlab, fontsize=ax_label_fs)
plt.ylabel(ylab, fontsize=ax_label_fs)
...
#plt.savefig("morphospace(1-2)_aspectratio.png")
```
...
averaged.to_csv("apple_metadata_0905.csv", index = False)
```
