...
ggpairs(all_data, columns = 16:20, columnLabels = c("width", "length", "area", "solidity", "asymmetry"), aes(colour = apple_id, alpha = 0.5), upper = list(continuous = "points"), diag = list(continuous = wrap("densityDiag", fill = "blue")))
```
...
apple_traits <- apple_traits %>% 
  group_by(nursery_id) %>% 
  mutate(across(all_of(c(10:20)), ~mean(.x, na.rm = TRUE), .names = "{col}"))
...
# plotting for 549 different apple_id
ggpairs(all_data, columns = 22:32, columnLabels = c("width", "length", "area", "solidity", "asymmetry", "PC1", "PC2", "PC3", "PC4", "PC5", "PC6"), aes(colour = apple_id, alpha = 0.5), upper = list(continuous = "points"), diag = list(continuous = wrap("densityDiag", fill = "blue")))

ggsave("pairs_plot2.png", plot = last_plot(), units = "in", width = 15, height = 12)
write.csv(all_data, file = "apple_metadata_0903.csv")
```
...
new_mdata = pd.read_csv("apple_metadata_avg_0903.csv")

#### SHOULD THESE NOW BE CALCULATED WITH THE REML ADJUSTED VALUES
# calculate standard deviations for each PC
PC1_std = new_mdata["PC1"].std()
PC2_std = new_mdata["PC2"].std()
PC3_std = new_mdata["PC3"].std()
PC4_std = new_mdata["PC4"].std()
PC5_std = new_mdata["PC5"].std()
PC6_std = new_mdata["PC6"].std()
...
avgdata <- read.csv("apple_metadata_0903.csv")
dataset <- read_excel("Dataset_S1.xlsx", sheet = "phenotype_data")

combined <- avgdata %>% left_join(dataset, by = 'apple_id')

write.csv(combined, file = "apple_metadata_0903.csv", row.names = FALSE)
```
...
averaged = pd.read_csv("apple_metadata_0903.csv")
...
averaged.to_csv("apple_metadata_0903.csv", index = False)
```
