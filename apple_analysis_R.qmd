...
#mdata.to_csv("apple_metadata_2.csv", index = False)
```
...
apple_data <- read.csv("apple_metadata_2.csv")
...
plot <- ggpairs(apple_test, columns = 26:30, columnLabels = c("width", "length", "area", "solidity", "asymmetry"), aes(colour = apple_id, alpha = 0.5), upper = list(continuous = "points"))

for(i in 1:plot$nrow) {
  for(j in 1:plot$ncol){
    if(i == j) {  # Only modify the diagonal plots (density plots)
      p <- ggplot(apple_data, aes_string(x = names(apple_data)[j + 10], fill = "as.factor(apple_id)")) + 
...
apple_metadata <- read.csv("apple_metadata_2.csv")
apple_info <- read_excel("Dataset_S1.xlsx", sheet = "phenotype_data")

# convert apple_id to have same type
apple_metadata$apple_id <- as.numeric(apple_metadata$apple_id)

# combine the apple info to the metadata
meta <- apple_metadata %>%
  left_join(apple_info) %>% 
  select(c(2:20, 22:26))

# save the file
write.csv(meta, file = "apple_metadata_2.csv")
```
